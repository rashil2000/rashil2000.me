...
export const imageUploader = async (directory, folder) => {
  ...
  try {
    ...
    document.getElementById('imageSpan').innerHTML = `The image is available at <a target='_blank' href='${absolutePath}'>${absolutePath}</a>.`;
...
export const updateBlog = async (title, description, content, slug, date) => {
  if (!confirm('Are all input fields correct?'))
    return null;
  ...
  try {
    const response = await fetch(`${baseUrl}/blogs/${slug}`, {
      method: 'PUT',
      headers: myHeaders,
      body: JSON.stringify({ title, description, content, date }),
      redirect: 'follow'
    });
    const result = await response.json();
    if (result._id)
...
export const updateProject = async (title, description, content, slug, github) => {
  if (!confirm('Are all input fields correct?'))
    return null;
  ...
  try {
    const response = await fetch(`${baseUrl}/projects/${slug}`, {
      method: 'PUT',
      headers: myHeaders,
      body: JSON.stringify({ title, description, content, github }),
      redirect: 'follow'
    });
    const result = await response.json();
    if (result._id)
